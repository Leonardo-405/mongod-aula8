...
// Definição do modelo Aluno com esquema
const Aluno = mongoose.model('Aluno', new mongoose.Schema({
    nome: String,
    idade: Number,
    ra: String,
    fk_idTurma: String // Campo para armazenar a referência à turma, se necessário
}));

// Definição do modelo Disciplina com esquema
...
// Rotas para operações com Aluno
...
// Rotas para operações com Disciplina
...
app.get('/disciplinas/:id', async (req, res) => {
    const disciplina = await Disciplina.findById(req.params.id).populate('alunos'); // Popula com os alunos associados, se necessário
...
app.delete('/disciplinas/:id', async (req, res) => {
    await Aluno.deleteMany({ fk_idTurma: req.params.id }); // Exclui alunos relacionados à disciplina
